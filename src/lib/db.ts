...
// Safe wrapper that handles missing Firebase gracefully
const safeCall = <T>(fn: (...args: any[]) => Promise<T>, fallback: T = null as any): Promise<T> => {
  if (!adminApp) {
    console.warn('Database call skipped - Firebase not initialized');
